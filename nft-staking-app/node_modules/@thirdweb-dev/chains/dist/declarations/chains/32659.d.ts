declare const _default: {
    readonly chain: "FSN";
    readonly chainId: 32659;
    readonly explorers: readonly [{
        readonly name: "fsnscan";
        readonly url: "https://fsnscan.com";
        readonly standard: "EIP3091";
        readonly icon: {
            readonly url: "ipfs://QmSAFx34SKNi7a139agX12f68oBMo2Ktt9c8yD8aFa14gd";
            readonly width: 48;
            readonly height: 51;
            readonly format: "svg";
        };
    }];
    readonly faucets: readonly [];
    readonly features: readonly [{
        readonly name: "EIP155";
    }, {
        readonly name: "EIP1559";
    }];
    readonly icon: {
        readonly url: "ipfs://QmX3tsEoj7SdaBLLV8VyyCUAmymdEGiSGeuTbxMrEMVvth";
        readonly width: 31;
        readonly height: 31;
        readonly format: "svg";
    };
    readonly infoURL: "https://fusion.org";
    readonly name: "Fusion Mainnet";
    readonly nativeCurrency: {
        readonly name: "Fusion";
        readonly symbol: "FSN";
        readonly decimals: 18;
    };
    readonly networkId: 32659;
    readonly rpc: readonly ["https://fusion.rpc.thirdweb.com/${THIRDWEB_API_KEY}", "https://32659.rpc.thirdweb.com/${THIRDWEB_API_KEY}", "https://mainnet.fusionnetwork.io", "wss://mainnet.fusionnetwork.io"];
    readonly shortName: "fsn";
    readonly slip44: 288;
    readonly slug: "fusion";
    readonly testnet: false;
};
export default _default;
//# sourceMappingURL=32659.d.ts.map