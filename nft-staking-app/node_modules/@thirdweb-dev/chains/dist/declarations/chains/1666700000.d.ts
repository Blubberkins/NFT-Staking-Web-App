declare const _default: {
    readonly chain: "Harmony";
    readonly chainId: 1666700000;
    readonly explorers: readonly [{
        readonly name: "Harmony Testnet Block Explorer";
        readonly url: "https://explorer.testnet.harmony.one";
        readonly standard: "EIP3091";
    }];
    readonly faucets: readonly ["https://faucet.pops.one"];
    readonly infoURL: "https://www.harmony.one/";
    readonly name: "Harmony Testnet Shard 0";
    readonly nativeCurrency: {
        readonly name: "ONE";
        readonly symbol: "ONE";
        readonly decimals: 18;
    };
    readonly networkId: 1666700000;
    readonly rpc: readonly ["https://harmony-testnet-shard-0.rpc.thirdweb.com/${THIRDWEB_API_KEY}", "https://1666700000.rpc.thirdweb.com/${THIRDWEB_API_KEY}", "https://api.s0.b.hmny.io"];
    readonly shortName: "hmy-b-s0";
    readonly slug: "harmony-testnet-shard-0";
    readonly testnet: true;
};
export default _default;
//# sourceMappingURL=1666700000.d.ts.map