...
import { s as setAnalyticsHeaders } from '../../../../dist/headers-fc6aaa21.browser.esm.js';
...
import { chainIdToThirdwebRpc, checkContractWalletSignature } from '../../../wallets/abstract/dist/thirdweb-dev-wallets-evm-wallets-abstract.browser.esm.js';
...
Code: ${errorCode}`;
  ...
  async signMessage(message) {
    ...
    let signature;
    try {
      const provider = new providers.JsonRpcProvider(chainIdToThirdwebRpc(chainId, this.config.clientId), chainId);
    ...
    const isValid = await checkContractWalletSignature(message, signature, address, chainId);
    if (isValid) {
      return signature;
    } else {
