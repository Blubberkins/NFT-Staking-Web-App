...
var headers = require('../../../../dist/headers-b46f30b9.cjs.prod.js');
...
Code: ${errorCode}`;
  ...
  async signMessage(message) {
    ...
    let signature;
    try {
      const provider = new ethers.providers.JsonRpcProvider(evm_wallets_abstract_dist_thirdwebDevWalletsEvmWalletsAbstract.chainIdToThirdwebRpc(chainId, this.config.clientId), chainId);
    ...
    const isValid = await evm_wallets_abstract_dist_thirdwebDevWalletsEvmWalletsAbstract.checkContractWalletSignature(message, signature, address, chainId);
    if (isValid) {
      return signature;
    } else {
